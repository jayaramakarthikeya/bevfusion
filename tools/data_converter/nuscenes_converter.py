...
def create_nuscenes_infos(
    ...
    train_nusc_infos, val_nusc_infos = _fill_trainval_infos(
        nusc, nusc_canbus, train_scenes, val_scenes, test, max_sweeps=max_sweeps
    )
...
def get_pose_matrix(pose):
    """Returns a 4x4 homogeneous transformation matrix from ego_pose."""
...
def get_trajectory_point(inital_pose,ego_pose,speed,T_lidar2ego):
    T_relative = np.linalg.inv(inital_pose) @ ego_pose
    T_lidar = np.linalg.inv(T_lidar2ego) @ T_relative
    return T_lidar

def _fill_trainval_infos(nusc, nusc_canbus, train_scenes, val_scenes, test=False, max_sweeps=10):
    ...
    for sample in mmcv.track_iter_progress(nusc.sample):
        ...
        if not os.path.exists(lidar_path):
            #print(f"{lidar_path} not found. Skipping.")
            continue
        ...
        trajectory = []
        trajectory_speed = []
        intial_world_pose = get_pose_matrix(pose_record)
        lidar2ego = get_pose_matrix(cs_record)
        while indx <= 4 and current_sample != "":
            ...
            scene_token = next_sample["scene_token"]
            scene = nusc.get('scene', scene_token)
            scene_name = scene['name']  
            can_file = os.path.join('/dataset/can_bus', f'{scene_name}.json')
            if not os.path.exists(can_file):
                if next_sample["next"] == "":
                    break
                nxt = nusc.get("sample", next_sample["next"])
                nxt_pose = nusc.get("ego_pose", nxt["data"]["LIDAR_TOP"])
                dt = (next_sample["timestamp"] - current_sample["timestamp"]) / 1e6
                dist = np.linalg.norm(np.array(nxt_pose["translation"]) - np.array(ego_pose["translation"]))
                speed_ms = dist / dt
            else:
                veh_msgs = nusc_canbus.get_messages(scene_name, 'vehicle_monitor') 
                utimes = np.array([m['utime'] for m in veh_msgs])
                speeds_kmh = np.array([m['vehicle_speed'] for m in veh_msgs])
                idx = np.argmin(np.abs(utimes - next_sample['timestamp']))
                matched_speed_kmh = speeds_kmh[idx]                    # km/h :contentReference[oaicite:5]{index=5}
                speed_ms = matched_speed_kmh / 3.6
            
            trajectory.append(get_trajectory_point(intial_world_pose,get_pose_matrix(ego_pose),speed_ms,lidar2ego)[np.newaxis,:])
            trajectory_speed.append(speed_ms)
            current_sample = next_sample
            indx += 1

        if len(trajectory) > 4:
            info["gt_trajectory"] = np.concatenate(trajectory[:-1], axis=0)
            info["gt_trajectory_speed"] = trajectory_speed[:-1]
            info["goal_point"] = trajectory[-1].squeeze(0)
            info["goal_point_speed"] = trajectory_speed[-1]
        else:
            continue
