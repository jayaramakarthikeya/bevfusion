...
class NuScenesDataset(Custom3DDataset):
    ...
    def get_data_info(self, index: int) -> Dict[str, Any]:
        ...
        # get gt_trajecoty for imitation
        data["gt_trajectory"] = torch.tensor(info["gt_trajectory"],dtype=torch.float32)
        data["gt_trajectory_speed"] = torch.tensor(info["gt_trajectory_speed"],dtype=torch.float32) 
        data["goal_point"] = torch.tensor(info["goal_point"],dtype=torch.float32)
        data["goal_point_speed"] = torch.tensor(info["goal_point_speed"],dtype=torch.float32)
