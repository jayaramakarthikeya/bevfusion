...
class GlobalRotScaleTrans:
    ...
    def transform_waypoint_pose(self,waypoint_pose, theta, translation, scale):
        # 1) build the global rotation matrix about +Z
        cos_t, sin_t = np.cos(theta), np.sin(theta)
        Rz = torch.tensor([
            [ cos_t, -sin_t, 0],
            [ sin_t,  cos_t, 0],
            [     0,      0, 1],
        ], dtype=waypoint_pose.dtype)

        # 2) split the original pose
        R_orig = waypoint_pose[:3, :3]   # 3×3 orientation
        t_orig = waypoint_pose[:3,  3]   # (tx,ty,tz)

        # 3) apply rotation to both R and t
        R_new = Rz @ R_orig

        # 4) apply scale to the translation term only (rotations aren’t “scaled”)
        t_scaled = (Rz @ t_orig) * scale

        # 5) then apply the same translation offset
        t_new = t_scaled + np.array(translation)

        # 6) re‐assemble into a 4×4
        new_pose = torch.eye(4, dtype=waypoint_pose.dtype)
        new_pose[:3, :3] = R_new
        new_pose[:3,  3] = t_new
        return new_pose
    ...
    def __call__(self, data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if self.is_train:
            ...
            waypoints = []
            for waypoint in data["gt_trajectory"]:
                waypoint = self.transform_waypoint_pose(waypoint, theta, translation, scale)
                waypoints.append(waypoint[np.newaxis,:])
            data["gt_trajectory"] = torch.concat(waypoints, dim=0)
            
            data["goal_point"] = self.transform_waypoint_pose(data["goal_point"], theta, translation, scale)
